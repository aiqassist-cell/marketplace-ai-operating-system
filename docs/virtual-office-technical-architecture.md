...
```mermaid
flowchart TD
    U["Владелец / команда QWEAR"] --> UI["Google Sheets / Telegram / простой web-интерфейс"]
    ...
    MKT --> SHEETS["Google Sheets: задачи, гипотезы, результаты"]
    SMM --> SHEETS
    STYLE --> SHEETS
    PHOTO --> SHEETS
    PROD --> SHEETS
...
### 1. Google Sheets: операционная база

Google Sheets используется не как интеллект, а как единый реестр:
...
Преимущество: команда уже работает с таблицами, данные видны и их легко проверять.

Ограничение: Sheets не должен хранить тяжелые фото и видео.
...
В Sheets хранятся ссылки на файлы Drive.
...
|---|---|---|
| Google Drive / Sheets | Рабочие данные, задачи, статусы, результаты, файлы | Обязательно |
...
4. n8n instance с доступом к Google Sheets, OpenAI и API маркетплейсов.
...
```text
Человек выбирает товар в Google Sheets
...
-> результаты записываются в Sheets и Drive
```
...
- одна Google-таблица;
- один лист `Товары в фокусе`;
- один лист `Задачи агентов`;
...
### Этап 4. Интерфейс Виртуального Офиса

Только после доказанного workflow можно строить отдельный интерфейс:
...
```text
Google Sheets / Drive = видимость и данные
